$(function(){
    ...
    $('body').off('click', '#testButton');
    $('body').on("click", '#testButton', function(event){
        layer.msg("hello", {icon: 1});
    });
    ...
    $('body').on("click", '.up-btn', function(event){
        ...
        var ctx = $("#ctx").val();
        var filetype = _this_up_btn.data('filetype');
        var file_dir = _this_up_btn.data('filedir');
        var title = "上传";
        var message = '<form method="POST" action="'+up_url+'" enctype="multipart/form-data" ><input type="file" name="imgFile" class="Uploads" /><input type="hidden" name="filetype" value="'+filetype+'" /><input type="hidden" name="fileDir" value="'+file_dir+'" /></form>';
        BootstrapDialog.confirm({
            title: title,
            message: message,
            btnCancelLabel: '取消',
                    ...
                    var ajax_option={
                        ...
                        success:function(data){
                            if(data.status == '1'){
                                ...
                                _this_up_btn.prev().attr("href", ctx+'/'+data.url);
                                _this_up_btn.prev().find('img').attr("src", ctx+'/'+data.url);
